...
class Sentence:
    ...
    def get_trimmed_sentence(self, s):
        _ , max_time_end = s.get_sentence_timestamp()
        trimmed_words = []
        for word in self.words:
            if word.time_end <= max_time_end:
                trimmed_words.append(word)
        return Sentence(words=trimmed_words)
    ...
    def get_sentence_timestamp(self):
        if len(self.words) > 0:
            return (self.words[0].time_start, self.words[-1].time_end)
